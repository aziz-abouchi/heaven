package io.github.treesitter.jtreesitter.heaven;

import java.lang.foreign.*;

public final class TreeSitterHeaven {
    ...
    private static final TreeSitterHeaven INSTANCE = new TreeSitterHeaven();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterHeaven() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_heaven");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-heaven");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-heaven");
