...
pub fn bridgeTheoremToProofEnv(self: *Shell, name: []const u8, stmt: []const u8) void {
    platform.dbg("DEBUG: bridging theorem name='{s}'\n", .{name});
    if (self.heaven.proof_core.theorems.get(name)) |thm| {
        platform.dbg("DEBUG: found in proof_core, lhs={d} rhs={d}\n", .{ thm.lhs, thm.rhs });
        self.proofs.theorem(name, stmt, thm.lhs, thm.rhs) catch |err| {
            platform.dbg("DEBUG: self.proofs.theorem failed: {s}\n", .{@errorName(err)});
        };
    } else {
        platform.dbg("DEBUG: NOT found in heaven.proof_core.theorems for name='{s}'\n", .{name});
    }
}
