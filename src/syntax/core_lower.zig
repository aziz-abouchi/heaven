    ...
    pub fn lowerExpr(self: *CoreLowerer, e: ast.Expr) anyerror!Id {
        switch (e) {
            ...
            .application => |apps| {
                ...
                const head_id = try self.lowerExpr(apps);
                var args_ids = std.ArrayList(Id).init(self.allocator);
                defer args_ids.deinit();
                for (apps[1..]) |arg| {
                    try args_ids.append(try self.lowerExpr(arg));
