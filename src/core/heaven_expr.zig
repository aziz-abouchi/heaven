...
pub const HeavenExpr = struct {
    ...
    /// Lower récursif d'une expression avant passage au noyau.
    /// Idempotent : si l'expression est déjà primitive, elle est retournée telle quelle.
    fn ensureLowered(self: *HeavenExpr, id: Id) !Id {
        return self.store.lowerRec(id);
    }
    ...
    // ─── Méthodes publiques d'évaluation ───
    ...
    pub fn evalId(self: *HeavenExpr, id: Id) !Id {
        const lowered = try self.ensureLowered(id);
        return self.engine.eval(lowered);
    }

    pub fn evalSExpr(self: *HeavenExpr, input: []const u8) !Id {
        const id = try self.parseSExpr(input);
        const lowered = try self.ensureLowered(id);
        return self.engine.eval(lowered);
    }

    pub fn evalTrace(self: *HeavenExpr, input: []const u8) !Id {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalTrace(lowered);
    }

    pub fn evalProfile(self: *HeavenExpr, input: []const u8) !Id {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalProfile(lowered);
    }
    ...
    pub fn typeOf(self: *HeavenExpr, input: []const u8) ![]u8 {
        ...
        defer infer.deinit();
        return infer.typeOf(lowered);
    }

    pub fn typeOfId(self: *HeavenExpr, id: Id) !Id {
        const lowered = try self.ensureLowered(id);
        var infer = types_mod.Infer.init(&self.store, self.allocator);
        defer infer.deinit();
        return infer.typeOf(lowered);
    ...
    pub fn typeOfSExpr(self: *HeavenExpr, input: []const u8) !Id {
        const id = try self.parseSExpr(input);
        const lowered = try self.ensureLowered(id);
        var infer = types_mod.Infer.init(&self.store, self.allocator);
        defer infer.deinit();
    ...
    pub fn simplify(self: *HeavenExpr, input: []const u8) ![]u8 {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return canon_mod.canonicalize(&self.store, self.allocator, lowered);
    }

    pub fn explain(self: *HeavenExpr, input: []const u8) ![]const u8 {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.explain(lowered);
    }
    ...
    pub fn evalTheorem(self: *HeavenExpr, input: []const u8) ![]u8 {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalTheorem(lowered);
    }

    pub fn evalProve(self: *HeavenExpr, input: []const u8) !Id {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalProve(lowered);
    }

    pub fn proveWith(self: *HeavenExpr, input: []const u8, strategy: []const u8) !Id {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.proveWith(lowered, strategy);
    }

    // ─── Transformation et optimisation ───

    pub fn evalTransform(self: *HeavenExpr, input: []const u8) !Id {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalTransform(lowered);
    }

    pub fn evalOptimize(self: *HeavenExpr, input: []const u8) !Id {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalOptimize(lowered);
    }

    // ─── Compilation MIR ───

    pub fn evalAsm(self: *HeavenExpr, input: []const u8) ![]const u8 {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalAsm(lowered);
    }

    pub fn evalMir(self: *HeavenExpr, input: []const u8) ![]const u8 {
        const id = try self.ensureLowered(try self.bridge.importExpr(input));
        const lowered = try self.ensureLowered(id);
        return self.engine.evalMir(lowered);
    }
    ...
    fn evalJs(self: *HeavenExpr, input: []const u8) ![]u8 {
        self.ensureInit();
        const id = try self.ensureLowered(self.parser.parseSExpr(input)) catch |err| {
