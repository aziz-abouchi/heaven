...
fn evalMagic(store: *Store, env: *Env, engine: *Engine, op: []const u8, args: []const Id, depth: u32) EvalError!Id {
    ...
    if (std.mem.eql(u8, op, "*")) return evalBinary(store, a, b, .mul);
    if (std.mem.eql(u8, op, "/")) return evalBinary(store, a, b, .div);
    if (std.mem.eql(u8, op, "%")) return evalBinary(store, a, b, .mod);
    if (std.mem.eql(u8, op, "&")) return evalBinary(store, a, b, .and_op);
    if (std.mem.eql(u8, op, "|")) return evalBinary(store, a, b, .or_op);
    if (std.mem.eql(u8, op, "=")) return evalCmp(store, a, b, .eq);
    if (std.mem.eql(u8, op, "!=")) return evalCmp(store, a, b, .neq);
    if (std.mem.eql(u8, op, "<")) return evalCmp(store, a, b, .lt);
    if (std.mem.eql(u8, op, ">")) return evalCmp(store, a, b, .gt);
